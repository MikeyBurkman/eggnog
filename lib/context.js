	...
	function buildResolver(mapping, resolvedDeps) {
		...
		var self = {
			...
			import: function(depId) {
				// TODO: This could be optimized a bit still
				var normId = normalizeId(depId);
				if (utils.contains(locals, normId)) {
					return self.importLocal(depId);
				} else if (utils.contains(externals, depId)) {
					return self.importExt(depId);
				} else if (utils.contains(globals, depId)) {
					return self.global(depId);
				} else {
					// TODO suggestions
					throw 'Unknown dependency: ' + depId;
				}
			},
			...
			importExt: function(extId) {
				if (!utils.contains(externals, extId)) {
					var msg = 'Could not find external dependency [' + extId + '] from module [' + mapping.id + ']';
					throw buildMissingDepMsg(msg, extId, externals);
				}
				return externalResolverFn(extId);
			},
			global: function(globalId) {
				if (!utils.contains(globals, globalId)) {
					var msg = 'Could not find global [' + globalId + '] from module [' + mapping.id + ']';
					throw buildMissingDepMsg(msg, globalId, globals);
				}
				return globalResolverFn(globalId);
			}
		};
